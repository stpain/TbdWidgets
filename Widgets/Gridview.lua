...
TBDGridviewMixin = {}
function TBDGridviewMixin:OnLoad()
    self.data = {}
    self.frames = {}
    self.itemMinWidth = 0
    self.itemMaxWidth = 0
    ...
    -- TBDScrollBarScrollDownButton.Normal:SetAtlas("minimal-scrollbar-arrow-bottom-down")
    -- TBDScrollBarScrollDownButton:SetSize(17, 11)
end

function TBDGridviewMixin:InitFramePool(type, template)
    self.framePool = CreateFramePool(type, self.scrollChild, template);
end

function TBDGridviewMixin:SetMinMaxSize(min, max)
    self.itemMinWidth = min;
    self.itemMaxWidth = max;
end

function TBDGridviewMixin:SetFixedColumnCount(count)
    self.fixedColumnCount = count;
end

function TBDGridviewMixin:InsertCustomFrame(frame)
    if self.frames then
...
function TBDGridviewMixin:Insert(info)
...
function TBDGridviewMixin:RemoveFrame(frame)
    local key;
...
function TBDGridviewMixin:GetItemSize()
...
function TBDGridviewMixin:UpdateLayout()
    self:GetItemSize()
...
function TBDGridviewMixin:GetFrames()
    return self.frames;
end