...
TbdWidgetsGridviewMixin = {}
function TbdWidgetsGridviewMixin:OnLoad()
    self.data = {}
    self.frames = {}
    self.itemMinWidth = 0
    self.itemMaxWidth = 0
...
function TbdWidgetsGridviewMixin:Insert(info)
...
function TbdWidgetsGridviewMixin:RemoveFrame(frame)
    local key;
...
function TbdWidgetsGridviewMixin:InsertTable(tbl)

end

function TbdWidgetsGridviewMixin:Flush()
    self.data = {}
    for k, f in ipairs(self.frames) do
        if f.ResetDataBinding then
            f:ResetDataBinding()
...
function TbdWidgetsGridviewMixin:GetItemSize()
...
function TbdWidgetsGridviewMixin:UpdateLayout()
    self:GetItemSize()
...
function TbdWidgetsGridviewMixin:GetFrames()
    return self.frames;
end